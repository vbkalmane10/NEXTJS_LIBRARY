            ...
            >
              <div className="flex items-center justify-between border-b pb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{t("phoneNumber")}: {user.phoneNumber}</p>
                  <p className="text-sm text-gray-500">{t("membershipStatus")}: {user.membershipStatus}</p>
                </div>
          ...
          <Table className="w-full border-separate border-spacing-y-2">
            <TableHeader className="bg-gray-200 text-gray-700">
              <TableRow>
                ...
                <TableHead className="px-4 py-5 font-medium">{t("name")}</TableHead>
                <TableHead className="px-4 py-5 font-medium">{t("email")}</TableHead>
                <TableHead className="px-4 py-5 font-medium">{t("phoneNumber")}</TableHead>
                <TableHead className="px-4 py-5 font-medium">{t("address")}</TableHead>
                <TableHead className="px-4 py-5 font-medium">{t("membershipStatus")}</TableHead>
            ...
            <TableBody className="bg-white">
              {users?.map((user, index) => (
                <TableRow key={user.id} className="hover:bg-gray-100 transition-colors duration-200">
                  <TableCell className="whitespace-nowrap py-3 pl-6 pr-3 text-gray-700">{index + 1}</TableCell>
                  <TableCell className="whitespace-nowrap px-3 py-3 text-gray-700">
                    {user.firstName} {user.lastName}
                  </TableCell>
                  <TableCell className="whitespace-nowrap px-3 py-3 text-gray-700">{user.email}</TableCell>
                  <TableCell className="whitespace-nowrap px-5 py-3 text-gray-700">{user.phoneNumber}</TableCell>
                  <TableCell className="whitespace-nowrap px-5 py-3 text-gray-700">{user.address}</TableCell>
                  <TableCell className="whitespace-nowrap px-10 py-3 text-gray-700">{user.membershipStatus}</TableCell>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>{user.firstName} will be deleted from our database</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
